...
with tf.device(dev3):
    ...
    T1 = []
    T1.append(tf.add(_X[0], _X[0]))
    T1.append(tf.add(_Y[0], _Y[0]))

Z1 = []
with tf.device(dev1):
    Z1.append(tf.matmul(T1[0], T1[0]))

with tf.device(dev2):
    Z1[0] = tf.matmul(Z1[0], Z1[0])

#with tf.device(dev3):
#    Z1.append(tf.add(Z1[0], Z1[0]))
...
for i in range(10):
    ...
    if i >= 2:
        ...
        trace_file = open('%s/timeline_matmul_%d.json' % (logPath, i), 'w')
